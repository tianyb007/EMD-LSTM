...
n_train = int(len(values) * 0.60)  # 取前60%的数据作为训练集 ### CHANGED
n_val = int(len(values) * 0.20)  # 取20%的数据作为验证集 ### CHANGED
train = values[:n_train, :]
val = values[n_train:n_train + n_val, :]  # 添加验证集 ### CHANGED
test = values[n_train + n_val:, :]  # 更改测试集起始位置 ### CHANGED
# 划分训练集、验证集和测试集的输入和输出 ### CHANGED
train_X, train_y = train[:, :-1], train[:, -1]
val_X, val_y = val[:, :-1], val[:, -1]  # 添加验证集输入和输出 ### CHANGED
test_X, test_y = test[:, :-1], test[:, -1]

train_X = train_X.reshape((train_X.shape[0], 1, train_X.shape[1]))
val_X = val_X.reshape((val_X.shape[0], 1, val_X.shape[1]))  # 修改验证集的形状 ### CHANGED
...
# fit network
history = model.fit(train_X, train_y, epochs=100, batch_size=4096, validation_data=(val_X, val_y), verbose=2,
                    shuffle=False)
model.save('C:/Users/tian/Desktop/data/water.h5')
...
# 保存预测结果到CSV文件 ### CHANGED
prediction_df = pd.DataFrame(inv_y_predict, columns=['Predicted Values'])
prediction_df.to_csv('C:/Users/tian/Desktop/data/predicted_values.csv', index=False)
