# 这段程序是处理水温的数据，按照年份的数据绘制出图形，并且输出三种格式的图片。输出图片的分辨率是300dpi
...
# 指定保存图像的路径
output_path = "C:/Users/tian/Desktop/data/test_pic/"
...
for year in range(start_year, end_year + 1):
    ...
    ax.plot(yearly_data["timestamp"], yearly_data["water_temperature"], color="black", linewidth=0.5, linestyle="solid")

    # 设置x轴日期格式
    ax.xaxis.set_major_formatter(mdates.DateFormatter("%Y-%m-%d"))
    plt.xlabel("Date")
    plt.ylabel("Water Temperature")
    plt.title(f"Water Temperature Data for {year}")

    # 调整布局以去除多余的白边
    fig.tight_layout()

    # 保存图形为 png、jpeg 和 svg 格式
    plt.savefig(f"{output_path}water_temperature_{year}.png", dpi=300)
    plt.savefig(f"{output_path}water_temperature_{year}.jpeg", dpi=300)
    plt.savefig(f"{output_path}water_temperature_{year}.svg", dpi=300)
    # 关闭图形
    plt.close(fig)
