...
# 读取txt文件格式
# 用这个画图，针对单一文件，输出原始数据的可视化图像，对999999和大于14的值进行处理，置为NaN。不对缺失值进行处理。
...
# 读取数据
data_file = "C:/Users/tian/Desktop/data/2015_zeroed.txt"
...
for line in lines:
    ...
    for idx, value in enumerate(values):
        if float(value) == 999999 or float(value) > 14:
            value = np.nan
        else:
            value = float(value)
        # 将分钟数据添加到时间戳中
...
plt.title("Water Temperature Data 2007-2022")
# plt.grid()
